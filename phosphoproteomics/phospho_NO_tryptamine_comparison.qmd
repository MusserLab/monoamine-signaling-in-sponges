...
# Note: limma_results_annotated.rds (260217 re-render) already contains
# Zang_et_al_2026 gene names in the Gene.short column and name_type column.
# No additional gene annotation join is needed.
...
if (length(all_shared_sites) > 0) {
  ...
  heatmap_data <- tryp_phospho %>%
    ...
    # Create wide format (use mean if multiple peptides map to same site)
    pivot_wider(
  ...
  # Ensure all expected comparison columns exist (some may be absent if

  # no shared sites are hits/candidates for that comparison)
  expected_cols <- c("3min", "15min", "30min", "Δ3→15min", "Δ15→30min")
  for (col in expected_cols) {
    if (!col %in% names(heatmap_data)) {
      heatmap_data[[col]] <- NA_real_
    }
  }
