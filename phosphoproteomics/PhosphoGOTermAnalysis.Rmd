...
- `phosphoproteomics/data/sl_go_prost.tsv` — Gene-to-GO term mapping (PROST-based annotations)
...
### Figure assembly notes (Fig 4C)

The following post-processing steps were applied manually during figure assembly
and are not scripted here:

- **Ontology filtering:** Fig 4C shows Molecular Function (MF) and Cellular
  Component (CC) only. This script computes enrichment for all three ontologies
  (BP, MF, CC).
- **Keyword exclusion:** Terms containing "phosphorylation," "kinase activity,"
  "phosphotransferase," "kinase," or "signal transduction" were excluded from
  the visualization.
- **Top-term selection:** The top 6 most significant terms per ontology were
  selected for the final figure.
...
go_map <- read.delim(here("phosphoproteomics", "data", "sl_go_prost.tsv"),
                     header = FALSE, stringsAsFactors = FALSE)[, 1:2]
colnames(go_map) <- c("gene_id", "go_id")

# --- C. Gene lengths (for goseq bias correction) ---
# File has two named columns: 'length' and 'gene_names' (hyphen-format IDs).
# trimws() strips any trailing \r from Windows line endings.
gene_lengths <- read.delim(
  here("data", "GO_GeneLengths.tsv"),
...
run_goseq_for_comparison <- function(current_comparison_label, full_data,
  ...
  message(paste0("  [", current_comparison_label, "] Length matches: ", n_matched,
  ...
  # Run goseq
  pwf <- safe_nullp(genes_vector, matched_lengths, label = current_comparison_label)
...
# Run goseq
pwf <- safe_nullp(genes_vector, matched_lengths, label = "Global_Phospho_Combined")
GO_results <- goseq(pwf, gene2cat = go_map, use_genes_without_cat = FALSE)
