...
- `outs/transcriptomics/MA_Plots_Analysis_interaction_v2.pdf` — MA plots for all comparisons
...
pdf(file.path(output_dir, "MA_Plots_Analysis_interaction_v2.pdf"), width = 8, height = 6)

plotMA(res_1h, ylim = c(-5, 5), main = "Tryptamine vs DMSO (1h)")
plotMA(res_4h, ylim = c(-5, 5), main = "Tryptamine vs DMSO (4h)")
plotMA(res_Rec, ylim = c(-5, 5), main = "Tryptamine vs DMSO (Recovery)")
plotMA(res_Time_4vs1, ylim = c(-5, 5),
       main = "Time Interaction: 1h -> 4h\n(Diff in Tryptamine relative to DMSO)")
plotMA(res_Time_Recvs4, ylim = c(-5, 5),
       main = "Time Interaction: 4h -> Rec\n(Diff in Tryptamine relative to DMSO)")

dev.off()
message("Plots saved to ", file.path(output_dir, "MA_Plots_Analysis_interaction_v2.pdf"))
```
